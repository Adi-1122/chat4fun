...
(() => {
  ...
  const Setting = {
    actionBar() {
      ...
      element.innerHTML = `<button id="back-dashboard"><i class="fa fa-arrow-left"></i></button>${lang.setting}`;
    ...
    accountSect(user) {
      ...
      const profilePicture = element.querySelector(
        `[data-account="profile-picture"]`
      );
      const profileUsername = element.querySelector(
        `[data-account="profile-username"]`
      );
      ...
      get(ref(db, "users/" + currentID)).then((data) => {
        ...
        profilePicture.style.backgroundImage = `url(${
          s.pictureURL || s.photoURL
        })`;
        profileUsername.innerText = s.username
          ? `@${s.username}`
          : s.displayName;
      ...
      const editPhoto = () => {
        ...
        element.onchange = () => {
          // Check File Extension and Size before Upload Photo

          const ext = element.value.slice(
            (Math.max(0, element.value.lastIndexOf(".")) || Infinity) + 1
          );
          ...
          if (file.size > 2102394)
            return popup.alert(
              `${lang.failed_size_1} ${bulat} ${lang.failed_size_2}`
            );
      ...
      const uploadPhoto = (file, ext) => {
        // Upload Photo Profile to Firebase Storage
