...
(() => {
  ...
  const getUserID = (id) => {
    if (id == currentID)
      return (window.location.href = `${window.location.origin}/Online/user/setting/`);
  ...
  const createElement = {
    // Create element with data and key (id)
    header() {
      const element = document.createElement("h1");
      element.innerHTML = `
                <button id="back-chat"><i class="fa fa-arrow-left"></i></button>
                ${lang.user_detail}
            `;
      container.appendChild(element);

      const back = element.querySelector("#back-chat");
      back.onclick = () =>
        (window.location.href = `${window.location.origin}/Online/chat/?r=user&gid=${urlResult}`);
    },
    main(s, id) {
      ...
      element.innerHTML = `
                <div class="User-name">
                    <h2 data-user="username" style="color: white;">Loading</h2>
                    <p data-user="gid" style="color: white;">Loading</p>
                </div>
      ...
      profileGlobalID.innerText = `Global ID: ${urlResult}`;
