...
(() => {
  ...
  const roomUser = async (urlParam) => {
    let id = urlParam.get("gid") || "none";

    if (id == "none")
      return popup.alert({
        msg: lang.group_id_failed,
        onyes: () =>
          (window.location.href =
            window.location.origin + "/Online/dashboard/"),
      });

    if (id == currentID)
      return popup.alert({
        msg: lang.cannot_self_chat,
        onyes: () =>
          (window.location.href =
            window.location.origin + "/Online/dashboard/"),
      });
    ...
    const userSubmission = (people) => {
      ...
      get(theirDB).then((privates) => {
        privates.forEach((privated) => {
          ...
          if (
            (s.first == theirID.first && s.second == theirID.second) ||
            (s.first == theirID.second && s.second == theirID.first)
          ) {
        ...
        if (isChatBefore === false) {
          ...
          set(child(theirDB, roomKey), theirID)
            .then(() => {
  ...
  const createElement = {
    header(key, people) {
      ...
      element.querySelector(`#back-dashboard`).onclick = () =>
        (window.location.href = `${window.location.origin}/Online/dashboard/`);
      ...
      detailElement.onclick = () => {
        if (currentRoom == "private") {
          window.location.href = `${window.location.origin}/Online/user/?gid=${people.key}`;
        } else if (currentRoom == "group") {
          window.location.href = `${window.location.origin}/Online/group/?gid=${people.key}`;
        }
      };
    ...
    main(key) {
      ...
      const showChat = (limitation, roomFixed) => {
        onChildAdded(roomFixed, (chat) => {
          ...
          untaian.id = chat.key;
          untaian.classList.add(`chatroom-${chat.key}`);
          ...
          get(ref(db, `users/${s.gid}`)).then((u) => {
            ...
            Username.onclick = () => {
              window.location.href = `${window.location.origin}/Online/user/?gid=${s.gid}`;
            };
          ...
          Option.innerHTML = `<i class="fa fa-ellipsis-h"></i>`;
          Option.onclick = () => {
            ...
            if (s.gid == currentID) {
              if (s.type === undefined) {
                ...
                Option_Reply.innerHTML = `<i class="fa fa-reply"></i> ${lang.reply}`;
                ...
                Option_Edit.innerHTML = `<i class="fa fa-edit"></i> ${lang.edit}`;
                ...
                Option_Trash.innerHTML = `<i class="fa fa-trash"></i> ${lang.delete}`;
                ...
                Chat_Message_Image_Img.onerror = () =>
                  (Chat_Message_Image_Img.src = "../data/img/error.jpg");
                ...
                Chat_Message.innerHTML += `<chatroom-file src="${s.msg}">${s.desc}</chatroom-file>`;
                Chat_Preview.appendChild(Chat_Message);
                ...
                Chat_Message.innerHTML += `<chatroom-audio src="${s.msg}"></chatroom-audio>`;
                Chat_Preview.appendChild(Chat_Message);
            ...
            Attach_Cancel.innerHTML = `<i class="fa fa-times-circle"></i> ${lang.cancel}`;
          ...
          const Option_Action = {
            reply(Option_Child) {
              ...
              if (s.type === undefined) {
                ...
                const From_Reply_Desc = document.createElement("div");
                From_Reply_Desc.classList.add("re-desc");
                From_Reply_Desc.innerText = s.desc
                  ? s.desc.length > 30
                    ? `${s.desc.replace(/\n/g, " ")}...`
                    : s.desc.replace(/\n/g, " ")
                  : `[${lang.picture}]`;
                ...
                From_Reply_Desc.innerText =
                  s.desc.length > 30
                    ? `${s.desc
                        .replace(/\n/g, " ")
                        .substring(0, 15)}...${s.desc.slice(
                        (Math.max(0, s.desc.lastIndexOf(".")) || Infinity) - 5
                      )}`
                    : s.desc;
                From_Reply.appendChild(From_Reply_Desc);
              } else if (s.type === "audio") {
              ...
              Cancel_Reply_Symbol.classList.add(
                "fa-duotone",
                "fa",
                "fa-times-circle"
              );
            ...
            edit(Option_Child) {
              ...
              popup.prompt({
                ...
                onyes: (res) => {
                  ...
                  if (res.length > 300)
                    return popup.alert(
                      `${lang.your_text_1} ${res.length}<br/>${lang.your_text_2}`
                    );

                  update(
                    ref(db, `${currentRoom}/${key}/${chat.key}`),
                    {
                      msg: res
                        .replace(/^\s+/g, "")
                        .replace(/ +(?= )/g, "")
            ...
            trash(Option_Child) {
              ...
              const deleting = () => {
                ...
                if (f === "image" || f === "file" || f === "audio") {
                  deleteObject(sref(storage, s.path))
                    .then(() => {
          ...
          (() => {
            ...
            if (s.type === undefined) {
              ...
              Img.onerror = () =>
                Img.setAttribute("src", "../data/img/error.jpg");
              ...
              if (s.desc) {
                Check_Message_Length(Message, s.desc, 100);
                Middle.appendChild(Message);
              }
              Img.onload = () => {
              ...
              Middle.innerHTML += `<chatroom-file src="${s.msg}">${s.desc}</chatroom-file>`;
            } else if (s.type === "audio") {
              Middle.innerHTML += `<chatroom-audio src="${s.msg}"></chatroom-audio>`;
            } else {
          ...
          (() => {
            ...
            const one = `${time.split("")[11]}${time.split("")[12]}`;
            const two = `${time.split("")[3]}${time.split("")[4]}`;
            const three = `${time.split("")[9]}${time.split("")[10]}`;
            const four = `${time.split("")[1]}${time.split("")[2]}`;
            const five = `${time.split("")[5]}${time.split("")[6]}${
              time.split("")[7]
            }${time.split("")[8]}`;
            let utcDate = `${five}/${four}/${three} ${one}:${two}:00 UTC`;
                  ...
                  get(ref(db, `users/${reply.val().gid}`)).then((u) => {
                    ...
                    if (t === undefined) {
                      ...
                      if (c.desc) {
                        ...
                        ReDesc.innerHTML = `<i class="fa fa-image"></i> ${lang.picture}`;
                      }
                      From.append(ReName, ReDesc);
                      Replied.appendChild(From);
                    } else if (t === "audio") {
                      ...
                      const ReDesc = document.createElement("p");
                      ReDesc.classList.add("re-desc");
                      ReDesc.innerHTML = `<i class="fa fa-microphone"></i> ${lang.voice_chat}`;
                      ...
                      ReDesc.innerHTML =
                        c.desc.length > 20
                          ? `<i class="fa fa-file"></i> ${c.desc.substring(
                              0,
                              10
                            )}...${c.desc.slice(
                              (Math.max(0, c.desc.lastIndexOf(".")) ||
                                Infinity) - 5
                            )}`
                          : `<i class="fa fa-file"></i> ${c.desc}`;
                      ...
                      const ReDesc = document.createElement("p");
                      ReDesc.classList.add("re-desc");
                      ReDesc.innerHTML = `<i class="fa fa-comment-question"></i> ${lang.deleted}`;
      ...
      mainArea.onscroll = () => {
        if (
          mainArea.scrollHeight - mainArea.scrollTop <
          mainArea.clientHeight + 100
        ) {
          ...
          if (!scrollNow_Before) {
            ...
            scrollNow.innerHTML = `<i class="fa fa-chevron-down"></i>`;
            scrollNow.onclick = () =>
              (mainArea.scrollTop = mainArea.scrollHeight);
      ...
      (() => {
        ...
        if (last_key && last_key !== null) {
          showChat(
            true,
            query(
              ref(db, `${currentRoom}/${key}`),
              limitToLast(50),
              orderByKey(),
              startAt(`${(last_key + 1).toString()}`)
            )
          );
        } else {
          showChat(
        ...
        (() => {
          if (!Changed.querySelector(".sender .bottom .edited")) {
            Changed.querySelector(
              ".sender .bottom"
            ).innerHTML += `<p class="edited c-yellow">${lang.edited}</p>`;
          }
          const ReplyChanged = document.querySelectorAll(
            `.replyto-${chat.key}`
          );
          if (ReplyChanged) {
            ReplyChanged.forEach((changeElement) => {
              Check_Message_Length(
                changeElement.querySelector(".from .re-desc"),
                chat.val().msg,
                100
              );
            });
          ...
          if (chat.val().type === undefined) {
            Check_Message_Length(
              Changed.querySelector(`.sender .middle p`) ||
                Changed.querySelector(`.sender .middle`),
              chat.val().msg,
              100
            );
          } else {
            if (Changed.querySelector(`.sender .middle`)) {
              Changed.querySelector(
                `.sender .middle`
              ).innerHTML = `<p><i class="c-yellow">${lang.has_been_deleted}</i></p>`;
            }
            const imageChanged = Changed.querySelector(
              ".sender .middle .picture"
            );
            if (imageChanged) {
              imageChanged.remove();
    ...
    footer(key) {
      ...
      const typingLogic = () => {
        ...
        if (textarea.value.length >= 1) {
          ...
          micButton.classList.add("fa-angle-right");
          textSend = true;
        } else {
          micButton.classList.remove("fa-angle-right");
      ...
      sendBtn.onclick = async () => {
        ...
        if (textarea.value.length > 300)
          return popup.alert(
            `${lang.your_text_1} ${textarea.value.length} ${lang.your_text_2}`
          );

        let savedText = textarea.value;

        /*
                const apiKey = "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"; //start
                const preText = 
                ...
                if(chatbotResponse.includes("Yes")){ //if chatbot response includes yes
                    popup.alert("Please use appropriate language")
                } else{  //end
*/
        ...
        untaian.innerHTML = `
                ...
                `;
        untaian.querySelector(`.sender .middle p`).innerText = savedText
          .replace(/^\s+/g, "")
          .replace(/ +(?= )/g, "")
          .replace(/^\n+/g, "")
          .replace(/\n /g, "\n")
        ...
        let one = new Date().getUTCHours();
        if (one < 10) one = "0" + one;
        let two = new Date().getUTCMinutes();
        if (two < 10) two = "0" + two;
        let three = new Date().getUTCDate();
        if (three < 10) three = "0" + three;
        let four = new Date().getUTCMonth();
        four = four + 1;
        if (four < 10) four = "0" + four;
        let five = new Date().getUTCFullYear();
        if (five < 10) five = "0" + five;
        ...
        if (reKey === null) {
          dataSend = {
            ...
            msg: savedText
              ...
              .replace(/\n /g, "\n")
              .replace(/\n+(?=\n\n)/g, ""),
            time: parseInt(`7${four}${two}${five}${three}${one}`),
          };
        } else {
          dataSend = {
            gid: currentID,
        ...
        set(
          ref(db, `${currentRoom}/${key}/${new Date().getTime().toString()}`),
          dataSend
        )
          .then(() => untaian.remove())
          .catch((err) => popup.alert(err));
      ...
      const voiceSend = (btnIcon, input) => {
        ...
        let device = navigator.mediaDevices.getUserMedia({ audio: true }); // get audio
        device.then((stream) => {
              ...
              if (recorder.state == "inactive") {
                ...
                reader.addEventListener(
                  ...
                  function () {
                    ...
                    const voiceUp = uploadBytesResumable(
                      sref(storage, path),
                      e.data
                    );
                    ...
                    voiceUp.on(
                      ...
                      (snapshot) => {
                        ...
                        uploadElement.querySelector(
                          `.sender .middle p`
                        ).innerHTML = `${lang.converting_voice}: ${progress}%`;
                      },
                      (err) => {
                      ...
                      () => {
                        ...
                        getDownloadURL(voiceUp.snapshot.ref).then(
                          (voiceURL) => {
                            let dataSend = null;
                            if (reKey === null) {
                              dataSend = {
                            ...
                            set(
                              ref(db, `${currentRoom}/${key}/${getTime}`),
                              dataSend
                            );
                            reKey = null;
  ...
  const createInput = {
    ...
    startPicture(key) {
      ...
      pictInput.onchange = () => {
        // picture input extension and size control
        const ext = pictInput.value.slice(
          (Math.max(0, pictInput.value.lastIndexOf(".")) || Infinity) + 1
        );
        const valid = ["jpg", "jpeg", "png", "webp"];
        const file = pictInput.files[0];
        const inp = file.size / 1053818;
        const size = Math.ceil(inp * Math.pow(10, 2)) / Math.pow(10, 2);

        if (!valid.includes(ext.toLowerCase()))
          return popup.alert(lang.failed_format);
        if (file.size > 2102394)
          return popup.alert(
            `${lang.failed_size_1} ${size} ${lang.failed_size_2}`
          );
        ...
        reader.onload = () => {
          ...
          const textReplace = () => {
            textPicture.value = textPicture.value
              .replace(/^\s+/g, "")
              .replace(/ +(?= )/g, "")
              .replace(/^\n+/g, "")
              .replace(/\n /g, "\n")
          ...
          const cancel = element.querySelector(
            `.buttons [data-button="cancel-file"]`
          );
          cancel.onclick = () => {
            this.end();
            pictInput.remove();
          };
          const sendFile = element.querySelector(
            `.buttons [data-button="send-file"]`
          );
          sendFile.onclick = () => {
            if (textPicture.value.length > 300)
              return popup.alert(
                `${lang.your_text_1} ${textPicture.value.length} ${lang.your_text_2}`
              );
      ...
      const pictureSend = (desc, file, ext) => {
        ...
        pictureUp.on(
          ...
          (snapshot) => {
            ...
            uploadElement.querySelector(
              `.sender .middle p`
            ).innerHTML = `${lang.uploading}: ${progress}%`;
          },
          (err) => {
            return popup.alert(err);
          },
    ...
    startFile(key) {
      // file send start function
      const fileInput = document.createElement("input");
      fileInput.setAttribute("type", "file");
      fileInput.click();
      fileInput.onchange = () => {
        const ext5 = fileInput.value.slice(
          (Math.max(0, fileInput.value.lastIndexOf(".")) || Infinity) - 5
        );
        const ext = fileInput.value.slice(
          (Math.max(0, fileInput.value.lastIndexOf(".")) || Infinity) + 1
        );
        ...
        element.innerHTML = `
                    <div class="file-preview">
                        <p data-file="file-name"><i class="fa fa-file"></i> ${
                          file.name.length > 20
                            ? `${file.name.substring(0, 10)} ... ${ext5}`
                            : file.name
                        }</p>
                    </div>
                    <div class="buttons">
                        <button data-button="cancel-file"><i class="fa-duotone fa-circle-x"></i> ${
                          lang.cancel
                        }</button>
                        <button data-button="send-file"><i class="fa-duotone fa-circle-check"></i> ${
                          lang.send
                        }</button>
                    </div>
                `;
      ...
      const fileSend = (file, ext) => {
        ...
        fileUp.on(
          "state_changed",
          (snapshot) => {
            // Upload Progress Event Listener and time out event listener if upload failed or slow connection
            let progress = Math.floor(
              (snapshot.bytesTransferred / snapshot.totalBytes) * 100
            );
            uploadElement.querySelector(
      ...
      if (this.emojiElement === null) {
        ...
        textarea.onkeypress = () => {
          if (this.emojiElement !== null) {
            ...
            emojiInput.innerHTML = `<i class="fa fa-smile-o" style="font-size:24px"></i>`;
          }
        };
        textarea.focus();
        emojiInput.innerHTML = `<i class="fa fa-close"></i>`;
